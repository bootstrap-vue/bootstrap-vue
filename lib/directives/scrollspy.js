...
ScrollSpy.prototype.listen = function () {
    ...
    if (scroller) {
        if (scroller.tagName !== 'BODY') {
            eventOn(scroller, 'scroll', this, false);
        }
        eventOn(window, 'scroll', this, false);
        eventOn(window, 'orientationchange', this, false);
        eventOn(window, 'resize', this, false);
    }
...
ScrollSpy.prototype.unListen = function () {
    ...
    if (scroller) {
        if (scroller.tagName !== 'BODY') {
            eventOff(scroller, 'scroll', this, false);
        }
        eventOff(window, 'scroll', this, false);
        eventOff(window, 'orientationchange', this, false);
        eventOff(window, 'resize', this, false);
    }
...
ScrollSpy.prototype._getOffsetHeight = function () {
    ...
    return scroller.tagName === 'BODY' ? window.innerHeight : getBCR(scroller).height;
};
...
ScrollSpy.prototype._clear = function () {
    selectAll(this._selector, this._$el).filter(el => {
        if (hasClass(el, ClassName.ACTIVE)) {
            const href = getAttr(el, 'href');
    ...
    while (el) {
        ...
        if (el && el.previousElementSibling) {
            for (let i = 0; i < classes.length - 1; i++) {
                if (hasClass(el.previousElementSibling, classes[i])) {
