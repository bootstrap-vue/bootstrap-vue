...
function ScrollSpy(el, binding) {
    ...
    this._selector = [Selector.NAV_LINKS, Selector.LIST_ITEMS, Selector.DROPDOWN_ITEMS].join(",");
...
ScrollSpy.prototype._activate = function(target) {
    ...
    links.forEach(link => {
        if (link.classList.contains(ClassName.DROPDOWN_ITEM)) {
            ...
            this._setParentsSiblingActiveState(
                link,
                Selector.NAV_LIST_GROUP,
                [ClassName.NAV_LINK, ClassName.LIST_ITEM],
                true
            );
        }
    });
