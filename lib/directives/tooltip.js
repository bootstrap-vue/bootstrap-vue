...
function parseBindings(bindings) {
    ...
    if (typeof bindings.value === 'string') {
        ...
        config = assign(this.$config.value);
    }
    ...
    keys(bindings.modifiers).forEach(mod => {
        if (/^html$/i.test(mod)) {
            // Title contains HTML
            config.html = true;
        } else if (/^nofade$/i.test(mod)) {
            // no animation
            config.animation = false;
        } else if (/^(auto|top|bottom|left|right)$/i.test(mod)) {
            // placement of tooltip
            config.position = mod.toLowerCase();
...
export default {
    ...
    update(el, bindings, vnode) {
        if (bindings.oldValue !== bindsings.oldValue) {
            applyBVTT(el, bindings, vnode);
        }
    },
