...
//
// Which transitionend event does this browser use
//
...
ToolTip.prototype.updateConfig = function(el, bindings, vnode) {
    ...
    if (typeof bindings.value === 'string') {
        ...
        this.$config = assign({}, DEFAULT, value);
    }
    ...
    VALID_TRIGGERS.forEach(trigger => {
        if (bindings.modifiers[trigger] && !triggers.indexOf(trigger) > -1) {
            triggers = triggers.split(' ').concat(trigger).join(' ').replace(/\s+/g, ' ').trim();
        }
    });
    this.$config.trigger = triggers || DEFAULT_TRIGGERS;
...
ToolTip.prototype.show = function() {
    ...
    desc = desc.split(' ').concat(this.$id).join(' ').replace(/\s+/g,' ').trim();
...
ToolTip.prototype.handlePopperPlacementChange = function(data) {
    this.cleanTipClass();
    this.addAttachmentClass(data.placement);
};
...
ToolTip.prototype.getTipElement = function(msg) {
    ...
    const tip = holder.removeChild(holder.firstElementChild);
    if (tip) {
        tip.setAttribute('id', this.$id);
        tip.setAttribute('role', 'tooltip');
...
ToolTip.prototype.addAttachmentClass = function(attachment) {
     attachment = this.getAttachment(attachment || this.$config.placemnt);
