...
ToolTip.prototype.show = function() {
    ...
    if (!document.body.contains(this.$ooltip)) {
    ...
    const complete = () => {
        ...
        self.$fadetimer = null;
        if (self.transitionEndEvent) {
            const events = self.transitionEndEvent.split(/\s+/);
            events.forEach(event => {
                self.$tooltip.removeEventListener(event, complete);
            });
        }
