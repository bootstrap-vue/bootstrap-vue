    ...
    methods: {
        ...
        destroyTether() {
            if (this.$data._tether && !this.showState) {
                ...
                const regex = new RegExp("(^|[^-]\\b)(" + TETHER_CLASS_PREFIX + "\\S*)", "g");
                this.$data._trigger.className = this.$data._trigger.className.replace(regex, "");
            }
        },
        ...
        eventHandler(e) {
            ...
            if (
                this.normalizedTriggers.length > 1 &&
                this.debounce > 0 &&
                this.lastEvent !== null &&
                e.timeStamp <= this.lastEvent + this.debounce
            ) {
                return;
            }
            ...
            for (const trigger in TRIGGER_LISTENERS) {
                for (const event in TRIGGER_LISTENERS[trigger]) {
                    if (event === e.type) {
                        ...
                        if (
                            action === "toggle" ||
                            (this.triggerState && action === "hide") ||
                            (!this.triggerState && action === "show")
                        ) {
