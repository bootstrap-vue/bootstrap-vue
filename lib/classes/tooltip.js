...
class ToolTip {
    ...
    show() {
        ...
        const complete = () => {
            ...
            this.clearTransition(tip, transEvent, complete);
        ...
        this.transition(tip, transEvent, complete);
    };
    ...
    clearTransition (tip, transEvent, complete) {
        clearTimeout(this.$fadeEvent);
        this.$fadeTimeout = null
        if (transEvent) {
            transEvent.split(/\s+/).forEach(evt => {
                if (evt) {
                    tip.removeEventListener(evt, complete);
                }
            });
        }
   };
