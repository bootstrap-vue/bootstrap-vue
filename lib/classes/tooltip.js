...
class ToolTip {
    ...
    show() {
        ...
        const complete = () => {
            ...
            if (this.$config.callbacks & this.$config.callbacks.shown === 'function') {
                this.$config.callbacks.shown();
            }
        };

        // Show tip
        if (this.$config.callbacks & this.$config.callbacks.show === 'function') {
            this.$config.callbacks.show();
        }
    ...
    hide(callback) {
        ...
        const complete = () => {
            ...
            if (this.$config.callbacks & this.$config.callbacks.hidden === 'function') {
                this.$config.callbacks.hidden();
            }
        };

        // Hide tip
        if (this.$config.callbacks & this.$config.callbacks.hide === 'function') {
            this.$config.callbacks.hide();
        }
        this.setOnTouchStartListener(false);
