...
class ToolTip {
    ...
    constructor(element, config, $root) {
        // New tooltip object
        this.$fadetimeout = nill;
        this.$hovertimeout = nill;
        this.$hoverState = '';
        this.$activeTrigger = {};
        this.$popper = null;
    ...
    updateConfig(config) {
        ...
        config = assign({}, this.constructor.Default, config);
    ...
    show() {
        ...
        if (!this.isWithContent()) {
            // Don't display if we do not have content
            return;
        }
        ...
        this.addAttachmentClass(this.getAttachment(attachment));
        ...
        const transEvent = self.getTransitionEndEvent();
        ...
        const complete = () => {
            ...
            if (transEvent) {
                transEvent.split(/\s+/).forEach(evt => {
                    if(evt) {
                       self.$tip.removeEventListener(evt, complete);
                    }
                });
            }
    ...
    hide(callback) {
        ...
        const complete = () => {
            ...
            if (self.$hoverState !== HoverState.SHOW && tip.parentNode) {
    ...
    getTipElement() {
        this.$tip = this.$tip || compileHtml(this.$config.template) || compileHtml(this.constructor.Default.template);
        return this.$tip;
    };
