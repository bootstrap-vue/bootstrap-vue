...
class ToolTip {
    ...
    show() {
        ...
        this.transition(tip, transEvent, complete, ClassName.SHOW);
    };

    transition (tip, transEvent, complete, transClass) {
        if (tip.classList.contains(transClass)) {
            if (transEvent) {
                transEvent.split(/\s+/).forEach(evt => {
                    if (evt) {
    ...
    hide(callback) {
        ...
        this.transition(tip, transEvent, complete, ClassName.FADE);
