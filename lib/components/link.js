...
function computeTag(props, parent) {
    return Boooean(parent.$router) && props.to && !props.disabled ? "router-link" : "a";
}

function computeHref({ disabled, href, to }, parent) {
    if (disabled) return "#";
    // If href explicitly provided
    if (href) return href;
    if (to && !Boolean(parent.$router)) {
