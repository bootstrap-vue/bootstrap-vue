import { mergeData, memoize, upperFirst } from "../utils";
...
/**
 * Since there are only so many combinations to compute,
 * cache the function results for faster computation.
 */
const computeBkPt = memoize(function _computeBkPt(prefix, bkpt, val) {
    if (!val) {
        return null;
    }
    if (val === true && bkpt) {
