...
describe('img', async () => {
  ...
  it("all but blanks should  have 'src' starting with 'https://placeimg.com'", async () => {
    const { app: { $refs } } = window;
    [
      'default',
      'fluid',
      ...
      const img = $refs[ref]
      expect(img).toBeDefined()
      expect(img.getAttribute('src')).toContain('https://placeimg.com')
    });
    [
      'blank',
      'blankSize',
      ...
      const img = $refs[ref]
      expect(img).toBeDefined()
      expect(img.getAttribute('src')).not.toContain('https://placeimg.com')
    })
  })
