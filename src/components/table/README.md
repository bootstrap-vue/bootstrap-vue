...
(sticky) as the table boxy scrolls. Setting the prop to `true` (or no explicit value) will generate a
table that has a maximum height of `300px`. To specify a maximum height other than `300px`, set the
`sticky-header` prop to a valid CSS height (including units). 
...
- The sticky header feature uses CSS `position: sticky` to position the headings. As IE11 does not
  support `position: sticky`, the table heading will scroll with the table body.
