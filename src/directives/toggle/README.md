...
canceled (i.e. via `evt.preventDefault()` or `@click.prevent`), and if so, it will _not_ dispatch
the toggle event to the target(s).

Because of this, avoid placing `v-b-toggle` on a `<b-button>` or `<b-link>` that has the `href`
prop set to `'#'`, as these components (or components based on them) call `event.preventDefault()`
to stop the browser from scrolling to the top of the page.
